...
(`lingam_varma_bootstrap()`, `get_varma_probabilities()`,
`get_varma_paths()`), total effects (`estimate_varma_total_effect()`), and
residual normality diagnostics mirror their VAR-LiNGAM counterparts. In the
probability matrix, the first `1 + p` column blocks are the psi (lag)
matrices and the final `q` blocks are the omega (MA) matrices.
