...
Variable pairs suspected to be connected through an unobserved variable
(`NA` entries in the adjacency matrix) are drawn as dashed, arrowless
segments. Because CAM-UV returns 0/1 edge indicators rather than
coefficients, edge labels are hidden by default (`label_edges = FALSE`);
a constant "1" on every edge carries no information.
