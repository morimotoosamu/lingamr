# Authors and Citation

## Authors

- **Osamu Morimoto**. Author, maintainer, copyright holder.

- **T. Ikeuchi**. Copyright holder.

- **G. Haraoka**. Copyright holder.

- **M. Ide**. Copyright holder.

- **W. Kurebayashi**. Copyright holder.

- **S. Shimizu**. Copyright holder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/morimotoosamu/lingamr/blob/main/DESCRIPTION)

Morimoto O (2026). *lingamr: 'LiNGAM' Algorithms for Causal Discovery*.
R package version 0.1.0, <https://github.com/morimotoosamu/lingamr>.

    @Manual{,
      title = {lingamr: 'LiNGAM' Algorithms for Causal Discovery},
      author = {Osamu Morimoto},
      year = {2026},
      note = {R package version 0.1.0},
      url = {https://github.com/morimotoosamu/lingamr},
    }
